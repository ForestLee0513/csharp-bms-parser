...
namespace BMSParser
{
    public class PatternProcessor
    {
        ...
        public void AssignObjectsBeat(int measure, int channel, string value, BMSModel model)
        {
            ...
            for (int i = 0; i < value.Length - 1; i += 2)
            {
                ...
                if (channel >= (int)Channel.P2_NORMAL_START_POS && channel < (int)Channel.P1_INVISIBLE_START_POS)
                {
                    ...
                    if (model.Base == Define.BMSModel.Base.BASE62)
                    {
                        measures[measure].AddNotes(line, pos, Util.Decode.DecodeBase62(splittedValue), Define.BMSObject.PlayerSide.P2);
                    }
                    else
                    {
                        measures[measure].AddNotes(line, pos, Util.Decode.DecodeBase36(splittedValue), Define.BMSObject.PlayerSide.P2);
                    }
                }
        ...
        public void SetNoteRandom(BMSKey mode, int randomSeed, Define.RandomMode p1RandomMode, Define.RandomMode p2RandomMode)
        {
            BMSKey lineCountPerLane;
            if (mode == BMSKey.BEAT_10K)
            {
                lineCountPerLane = BMSKey.BEAT_5K;
            }
            else if (mode == BMSKey.BEAT_14K)
            {
                lineCountPerLane = BMSKey.BEAT_7K;
            }
            else
            {
                lineCountPerLane = mode;
            }
            ...
            if (p1RandomMode == Define.RandomMode.RANDOM)
            {
                ...
                foreach (KeyValuePair<double, Timestamp> timestamp in Timestamp)
                {
                    ...
                    Timestamp[timestamp.Key].P1Lane = randomizedNormalLane;
                    Timestamp[timestamp.Key].P1LongLane = randomizedLongLane;
                    Timestamp[timestamp.Key].P1MineLane = randomizedMineLane;
                    Timestamp[timestamp.Key].P1HiddenLane = randomizedHiddenLane;
                }
            }
            ...
            if (p2RandomMode == Define.RandomMode.RANDOM)
            {
                ...
                foreach (KeyValuePair<double, Timestamp> timestamp in Timestamp)
                {
                    ...
                    Timestamp[timestamp.Key].P2Lane = randomizedNormalLane;
                    Timestamp[timestamp.Key].P2LongLane = randomizedLongLane;
                    Timestamp[timestamp.Key].P2MineLane = randomizedMineLane;
                    Timestamp[timestamp.Key].P2HiddenLane = randomizedHiddenLane;
                }
            }
            // R-RANDOM //
